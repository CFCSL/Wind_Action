...
v_b2 = v_b_func().subs(db)
st.latex(latex(v_b2))
db['v_b'] = v_b_func().subs(db).rhs
...
c_prob2=c_prob_func().subs(db)
db['c_prob']=c_prob_func().subs(db)
...
z_val=st.number_input('$z=$',value= 1.00, min_value=0.0, step=0.01, format="%.3f")
...
v_m2=v_m_func().subs(z, z_val).subs(db)
...
#db['c_r']=c_r_func().subs(db).doit().rhs
c_r=N(c_r2.doit(),3)
